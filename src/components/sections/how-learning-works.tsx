    ...
    >
      <Container>
        ...
        <ol
          ...
          className="mt-14 grid gap-6 md:grid-cols-2 md:gap-8 lg:mt-20 lg:grid-cols-4 lg:gap-6"
        >
          {learningProcessSteps.map((step, index) => {
            ...
            return (
              ...
              <article className="group relative z-floating flex min-h-full flex-col rounded-large border border-border bg-background p-8 shadow-small transition-[border-color,box-shadow,transform] duration-normal ease-swq-out hover:-translate-y-1 hover:border-primary-200 hover:shadow-hover motion-reduce:transition-none lg:min-h-[22rem]">
                ...
                {index < learningProcessSteps.length - 1 ? (
                  <span
                    aria-hidden="true"
                    className="pointer-events-none absolute -right-6 top-9 z-0 hidden h-px w-6 bg-gold-200 lg:block"
                  />
                ) : null}
              </li>
            );
        ...
        <div className="mt-16 flex flex-col gap-6 rounded-large bg-primary-600 px-6 py-8 text-text-inverse shadow-medium md:flex-row md:items-center md:justify-between md:px-10 md:py-10 lg:mt-20 lg:px-12">
